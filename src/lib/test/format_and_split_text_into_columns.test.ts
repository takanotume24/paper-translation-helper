...
test("formatAndSplitTextIntoColumns - should apply all text processing rules correctly", () => {
...
test("formatAndSplitTextIntoColumns - should handle empty strings", () => {
    const result = formatAndSplitTextIntoColumns('', 100);
    expect(result).toEqual([['']]);
});

test("formatAndSplitTextIntoColumns - should process and split text correctly", () => {
